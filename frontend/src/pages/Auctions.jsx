  ...
  return (
    <>
      <style>{`
        ...
        .empty-state-wrap { display:flex; flex-direction:column; align-items:center; padding:90px 24px; grid-column:1 / -1; }
        .empty-heading { font-family:'Barlow Condensed',sans-serif; font-size:34px; font-weight:800; text-transform:uppercase; margin-bottom:12px; }
        .empty-sub { color:var(--muted); line-height:1.7; max-width:420px; margin:0 auto 28px; text-align:center; }
        .modal-overlay { position:fixed; inset:0; background:rgba(0,0,0,.72); display:flex; align-items:center; justify-content:center; padding:20px; z-index:110; }
        .modal-card { width:min(100%, 520px); background:var(--surface); border:1px solid var(--border); border-radius:22px; padding:28px; position:relative; }
        .modal-close { position:absolute; top:16px; right:18px; background:none; border:none; color:var(--muted); font-size:28px; cursor:pointer; }
        .modal-title { font-family:'Barlow Condensed',sans-serif; font-size:34px; font-weight:800; text-transform:uppercase; margin-bottom:10px; }
        .modal-sub { color:var(--muted); line-height:1.6; margin-bottom:18px; }
        .modal-field { margin-bottom:16px; }
        .modal-label { display:block; font-size:12px; color:var(--muted); text-transform:uppercase; letter-spacing:1px; margin-bottom:10px; }
        .modal-input { width:100%; background:#090909; color:var(--text); border:1px solid #232323; border-radius:14px; padding:16px; font-size:15px; }
        .modal-submit { width:100%; padding:13px 20px; font-size:14px; font-weight:600; border-radius:50px; background:var(--accent); color:var(--text); border:none; cursor:pointer; }
        .modal-submit:disabled { background:#262626; color:var(--muted); cursor:not-allowed; }
        .modal-feedback { margin-top:16px; font-size:14px; line-height:1.6; }
        .modal-feedback.success { color:var(--success); }
        .modal-feedback.error { color:var(--accent); }
        @media (max-width: 960px) {
          .user-strip, .filter-bar, .auctions-section { padding-left:20px; padding-right:20px; }
        }
      `}</style>
      ...
      <div className="user-strip">
        ...
        <a href="/signup" className="btn-ghost-sm" onClick={e => { e.preventDefault(); navigate('/signup'); }}>
          {currentUserId ? 'Switch / Reconnect Account' : 'Create Account'}
        </a>
      </div>
        ...
        <div className="auction-grid">
          {visible.length === 0 ? (
            <div className="empty-state-wrap">
              <div className="empty-heading">No Auctions Found</div>
              <p className="empty-sub">Once an auction is created, it will show up here as a card.</p>
              <button className="btn-view" style={{ width: 'auto', padding: '13px 28px' }} onClick={() => navigate('/create')}>Create First Auction</button>
            </div>
          ) : visible.map(auction => {
            ...
            return (
              <div key={auction.auction_id} className="auction-card" onClick={() => navigate(`/auctions/${auction.auction_id}`)}>
                <div className="card-image">
                  ...
                  <span className="card-placeholder" style={{ display: auction.image_url ? 'none' : 'block' }}>📷</span>
      ...
      {/* Bid Modal */}
      {bidModal && (
        <div className="modal-overlay" onClick={e => { if (e.target.className === 'modal-overlay') closeBidModal(); }}>
          <div className="modal-card">
            <button className="modal-close" onClick={closeBidModal}>×</button>
            <div className="modal-title">Place a Bid</div>
            <p className="modal-sub">
              Auction #{bidModal.auction.auction_id} · {bidModal.auction.camera_model || 'Camera'} · Minimum bid {fmtSGD(minBidForModal)}
            </p>
            <form onSubmit={submitBid}>
              <div className="modal-field">
                <label className="modal-label" htmlFor="bidAmount">Bid Amount (SGD)</label>
                <input
                  className="modal-input"
                  type="number"
                  id="bidAmount"
                  min={minBidForModal.toFixed(2)}
                  step="0.01"
              ...
              <button type="submit" className="modal-submit" disabled={bidSubmitting}>
                {bidSubmitting ? 'Submitting…' : 'Submit Bid'}
              </button>
              {bidFeedback.text && (
                <div className={`modal-feedback ${bidFeedback.cls}`}>{bidFeedback.text}</div>
              )}
            </form>
          </div>
        </div>
